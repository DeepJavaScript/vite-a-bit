<template>
    ...
    <div v-if="isPickerOn" class="picker">
      ...
      <div class="body">
        ...
        <p @click="pickDate" v-for="date in dayOnThisMonth" class="date">{{date}}</p>
      </div>
    </div>
	</p>
</template>
...
export default {
  ...
  computed: {
    date() {return this.dayInstance.getDate()},
    month() {return this.dayInstance.getMonth()},
    year() {return this.dayInstance.getFullYear()},
    emptyAmout() {return this.dayInstance.getDay()},
    dayOnThisMonth() {return new Date(this.date, this.month, 0).getDate()},
  },
  methods: {
    pickDate(event) {
      const dateString = (event.target.textContent.length < 2) ? '0'.concat(event.target.textContent) : event.target.textContent;
      this.dateValue = `${this.year}-${this.numToString(this.month + 1)}-${dateString}`;
    ...
    numToString(num) {
      if (typeof num !== 'number') throw 'Number to String error: input is not a Number';
